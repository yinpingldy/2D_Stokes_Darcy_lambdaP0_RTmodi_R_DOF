function result = assemble_matrix_from_line_integral_triangle_RT0_L(coefficient_function, edges, Gamma_inter_num, edge_condition, Stokes_t, P_partition, T_partition, N_basis_trial,  N_basis_test, nodes_stokes, number_of_edges, nodes_stokes_num, triangle_edge,...
    number_of_trial_local_basis, number_of_test_local_basis, number_of_Gamma_inter, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, lable_test, trial_derivative_degree, test_basis_type, test_derivative_degree_x, test_derivative_degree_y)
...
for n=1:number_of_Gamma_inter
    ...
    for alpha = 1:number_of_trial_local_basis
       for beta = 1:number_of_test_local_basis 
           ...
           j = triangle_edge(alpha,element_S);
           if any(Gamma_inter_num==j)
               j = find(Gamma_inter_num==j) + number_of_edges + nodes_stokes_num;
           end
           
           int_value = Gauss_quadrature_for_line_integral_trial_test_2D_RT0_L(coefficient_function, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, endpoint(:,1), endpoint(:,2), normal_vector, lable_test,...
               vertices, alpha, trial_derivative_degree, vertices, test_basis_type, beta, test_derivative_degree_x, test_derivative_degree_y);
