function [result] = Gauss_quadrature_for_surface_integral_u_error_triangle(element, uh, index_u,  exact_function, vertices,...
...
for i = 1:Gpn
    result = result + Gauss_weights_local_triangle(i) * (feval(exact_function, Gauss_nodes_local_triangle(i,1), Gauss_nodes_local_triangle(i,2))...
        - FE_solution_local_triangle_u(element, Gauss_nodes_local_triangle(i,1), Gauss_nodes_local_triangle(i,2),...
        uh, index_u, vertices, triangle, triangle_edge, Gamma_inter_num, number_of_edges, nodes_stokes_num, stokes_t, nodes_stokes, derivative_degree, basis_type, derivative_degree_x, derivative_degree_y))^2;
end
