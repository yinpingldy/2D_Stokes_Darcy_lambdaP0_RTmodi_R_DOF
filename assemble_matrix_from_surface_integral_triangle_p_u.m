function result = assemble_matrix_from_surface_integral_triangle_p_u(coefficient_function, P_partition, T_partition, N_basis_trial,  N_basis_test, T_basis_p, Stokes_t, nodes_stokes, number_of_edges, ...
...
for n=1:number_of_elements
    ...
    element_now = Stokes_t(n);
    
    for alpha = 1:number_of_trial_local_basis
       for beta = 1:number_of_test_local_basis      
          int_value = Gauss_quadrature_for_surface_integral_trial_test_triangle(coefficient_function,Gauss_weights_local_triangle,Gauss_nodes_local_triangle,vertices,basis_type_trial,alpha,derivative_degree_x_trial,derivative_degree_y_trial,basis_type_test,beta,derivative_degree_x_test,derivative_degree_y_test);
          i_node = T_partition(beta,n);
          i = find(nodes_stokes == i_node) + number_of_edges;
          j = T_basis_p(element_now);
