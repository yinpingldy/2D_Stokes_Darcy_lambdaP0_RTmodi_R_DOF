function [result] = FE_solution_error_infinity_norm_triangle(uh, accurate_function, left, right, bottom, top, h_partition, basis_type, derivative_degree_x, derivative_degree_y, Gauss_point_number)
...
if basis_type==2
    [~,T_basis]=generate_P_T_triangle(left,right,bottom,top,h_partition,2);
elseif basis_type==1
    T_basis=T_partition;
end
...
for n=1:number_of_elements
    ...
    [~,Gauss_point_local_triangle]=generate_Gauss_local_triangle(Gauss_coefficient_reference_triangle,Gauss_point_reference_triangle,vertices);
    
    temp=max(abs( feval(accurate_function, Gauss_point_local_triangle(:,1), Gauss_point_local_triangle(:,2)) -...
        FE_solution_local_triangle(Gauss_point_local_triangle(:,1), Gauss_point_local_triangle(:,2), uh_local, vertices, basis_type, derivative_degree_x, derivative_degree_y) ));    
    if temp>result
        result=temp;
    end
end