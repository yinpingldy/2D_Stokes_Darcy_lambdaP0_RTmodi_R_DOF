function result = assemble_matrix_from_line_integral_triangle_RT0(coefficient_function, edges, Gamma_inter_num, edge_condition, Stokes_t, P_partition, T_partition, N_basis_trial,  N_basis_test, triangle_edge, Gamma_stokes_num_full,...
    number_of_trial_local_basis, number_of_test_local_basis, number_of_Gamma_inter, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, trial_derivative_degree, test_derivative_degree)
...
for n=1:number_of_Gamma_inter
    ...
    for alpha = 1:number_of_trial_local_basis
       for beta = 1:number_of_test_local_basis
           ...
           if any(Gamma_stokes_num_full==i) || any(Gamma_stokes_num_full==j)
               result(i,j) = 0;
           else
               int_value = Gauss_quadrature_for_line_integral_trial_test_2D_RT0(coefficient_function, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, endpoint(:,1), endpoint(:,2), tan_vector,...
