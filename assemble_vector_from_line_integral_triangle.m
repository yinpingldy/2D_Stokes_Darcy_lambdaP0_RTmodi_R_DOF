function result = assemble_vector_from_line_integral_triangle(coefficient_function, edges, Gamma_inter_num, edge_condition, Stokes_t, P_partition, T_partition,  N_basis_test, nodes_stokes, number_of_edges,...
     number_of_test_local_basis, number_of_Gamma_inter, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, lable_n_or_t, lable_test,...
     basis_type_test, derivative_degree_x_test, derivative_degree_y_test, project_x_y)
...
for n=1:number_of_Gamma_inter
    ...
    for beta = 1:number_of_test_local_basis  
        if lable_n_or_t == 'n'
            int_value = Gauss_quadrature_for_line_integral_test_2D_proj_n(coefficient_function, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, normal_vector,...
                endpoint(:,1), endpoint(:,2), vertices, basis_type_test, beta, derivative_degree_x_test, derivative_degree_y_test, project_x_y);
        elseif lable_n_or_t == 't'
            int_value = Gauss_quadrature_for_line_integral_test_2D(coefficient_function, Gauss_weights_reference_1D, Gauss_nodes_reference_1D, tan_vector, lable_test,...
                endpoint(:,1), endpoint(:,2), vertices, basis_type_test, beta, derivative_degree_x_test, derivative_degree_y_test);
        end
        
        i_node = T_partition(beta,element_S);
